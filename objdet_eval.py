...
def measure_detection_performance(detections, labels, labels_valid, min_iou=0.5):
    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            box = label.box
            box_t = tools.compute_box_corners(box.center_x,box.center_y,box.width,box.length,box.heading)
            ...
            for detec in detections:
                ...
                idx,x,y,z,h,w,l,yaw = detec
                box_p = tools.compute_box_corners(x,y,w,l,yaw)
                ...
                poly_t = Polygon(box_t)
                poly_p = Polygon(box_p)
                itersection = poly_t.intersection(poly_p).area
                union = poly_t.union(poly_p).area
                iou = itersection / union
...
def compute_performance_stats(det_performance_all):
    ...
    false_positives = np.sum(pos_neg_arr[:,3])
